...
def handle(client):
    ...
    for file in files:
        if not file.name == "Lab data-source.json":
            ...
            if "Alkane Profile" in remaining_text:
                remaining_sections = remaining_text.split("Alkane Profile")
            else:
            ...
            cutpoint_df.columns = [
                "recovered_at_" + col.replace(",0", "").replace(".0", "") for col in cutpoint_df.iloc[0]
            ]
